--------------------------------------------------------
--  DDL for Function FN_FDS_RULE23_V2
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDS_RULE23_V2" 
/*********************************************************************************
...
BEGIN
    ...
    IF (V_POS_MODE = '05' OR V_POS_MODE = '90' OR V_POS_MODE = '91') AND P_REF_CDE NOT LIKE '%10%' AND P_REF_CDE NOT LIKE '%35%' THEN
        ...
        l_check_point := 2;
        SELECT
          COUNT(DISTINCT CNTR_CDE) INTO v_cnt
        FROM
          (SELECT
        ...
        IF v_cnt >= 2 THEN
            ...
            ------------------------------------
            l_check_point := 4;
            sql_stmt2 := 'INSERT INTO CCPS.FDS_CASE_HIT_RULE_DETAIL SELECT ''' || cre_tms || ''',''' || cre_tms || ''',''BACKEND_USR'',''' || case_id || ''',''RULE23'',f9_oa008_cre_tms,fx_oa008_used_pan,f9_oa008_mcc,p9_oa008_seq,f9_oa008_dt,f9_oa008_tm FROM FDS_TXN_DETAIL where substr(fx_oa008_pos_mode,1,2) in (''05'',''90'',''91'') AND f9_oa008_cre_tms >= ' || strt_tms ||' AND f9_oa008_cre_tms <= ' || p_end_tms || ' AND fx_oa008_used_pan = ''' || p_cardno || ''' ORDER BY f9_oa008_cre_tms desc';

--------------------------------------------------------
--  DDL for Function FN_FDS_RULE13_V3_UAT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDS_RULE13_V3_UAT" 
/***********************************************************************************
...
--	SELECT PX_OA008_PAN INTO V_PAN FROM FDS_TXN_DETAIL WHERE FX_OA008_USED_PAN=P_CARDNO AND P9_OA008_SEQ=P_SEQ AND F9_OA008_CRE_TMS=P_END_TMS;
    IF p_ref_cde LIKE '%13%' AND p_ref_cde NOT LIKE '%10%' AND p_ref_cde NOT LIKE '%35%' THEN
        ...
        SQL_STMT := 'SELECT count(1) FROM FPT.FDS_TXN_DETAIL T WHERE T.fx_oa008_ref_cde LIKE ''%13%'' AND T.FX_OA008_USED_PAN = '''||P_CARDNO||''' AND T.F9_OA008_CRE_TMS BETWEEN ' || STRT_TMS || ' AND ' ||P_END_TMS;
        ...
        IF V_CNT > 1 THEN--tu 2 giao dich tro len moi hit rule tao case
          ...
          --------------------------------------
          l_check_point := 3;
          sql_stmt2 := 'INSERT INTO FPT.FDS_CASE_HIT_RULE_DETAIL SELECT ''' || cre_tms || ''',''' || cre_tms || ''',''BACKEND_USR'',''' || case_id || ''',''RULE13'',f9_oa008_cre_tms,fx_oa008_used_pan,f9_oa008_mcc,p9_oa008_seq,f9_oa008_dt,f9_oa008_tm FROM FPT.FDS_TXN_DETAIL where f9_oa008_amt_req >= 0 and f9_oa008_cre_tms = ' || p_end_tms ||' AND fx_oa008_used_pan = ''' || p_cardno || '''';
          execute immediate sql_stmt2;
          l_check_point := 4;
          sql_stmt2 := 'INSERT INTO FPT.FDS_CASE_HIT_RULES VALUES(' || cre_tms || ',' || cre_tms || ',''BACKEND_USR'',''' || case_id || ''',''RULE13'')';
          execute immediate sql_stmt2;
          commit;
        ...
        ELSE
            SQL_STMT := 'SELECT count(1) FROM FPT.FDS_TXN_DETAIL T WHERE T.fx_oa008_ref_cde LIKE ''%13%'' AND T.FX_OA008_USED_PAN = '''||P_CARDNO||''' AND T.FX_OA008_MID = '''||P_MID||''' AND T.F9_OA008_CRE_TMS BETWEEN ' || STRT_TMS || ' AND ' ||P_END_TMS;
            EXECUTE IMMEDIATE SQL_STMT INTO V_CNT;

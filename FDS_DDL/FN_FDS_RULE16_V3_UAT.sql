--------------------------------------------------------
--  DDL for Function FN_FDS_RULE16_V3_UAT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDS_RULE16_V3_UAT" 
/***********************************************************************************
...
BEGIN
    ...
    IF p_ref_cde LIKE '%25%' AND p_ref_cde NOT LIKE '%10%' AND p_ref_cde NOT LIKE '%35%' THEN
      ...
      SQL_STMT := 'SELECT count(1) FROM FPT.FDS_TXN_DETAIL T WHERE T.fx_oa008_ref_cde LIKE ''%25%'' AND T.FX_OA008_USED_PAN = '''||P_CARDNO||''' AND T.F9_OA008_CRE_TMS BETWEEN ' || STRT_TMS || ' AND ' ||P_END_TMS;
      ...
      IF V_CNT > 1 THEN--tu 2 giao dich tro len moi hit rule tao case
        ...
        END IF;
        ----------------------------------------
        SELECT trim(p9_oa008_seq)||'-'||trim(fx_oa008_given_apv_cde)||'-'||trim(f9_oa008_stan)||'-'||trim(f9_oa008_mcc) INTO case_id FROM FPT.fds_txn_detail where f9_oa008_amt_req >= 0 AND fx_oa008_used_pan = p_cardno AND f9_oa008_cre_tms = p_end_tms AND P9_OA008_SEQ=P_SEQ;
        ...
        --------------------------------------
        l_check_point := 3;
        sql_stmt2 :='INSERT INTO FPT.FDS_CASE_HIT_RULE_DETAIL SELECT ''' || CRE_TMS || ''', ''' || CRE_TMS || ''', ''BACKEND_USR'', ''' || CASE_ID || ''', '''||V_RULENAME||''', F9_OA008_CRE_TMS, FX_OA008_USED_PAN, F9_OA008_MCC, P9_OA008_SEQ, F9_OA008_DT, F9_OA008_TM FROM FPT.FDS_TXN_DETAIL WHERE F9_OA008_CRE_TMS = '|| P_END_TMS ||' AND FX_OA008_USED_PAN = ''' || P_CARDNO || ''' ORDER BY F9_OA008_CRE_TMS DESC';
        ...
        RETURN 'DONE';
      ELSE
        SQL_STMT := 'SELECT count(1) FROM FPT.FDS_TXN_DETAIL T WHERE T.fx_oa008_ref_cde LIKE ''%25%'' AND T.FX_OA008_USED_PAN = '''||P_CARDNO||''' AND T.FX_OA008_MID = '''||P_MID||''' AND T.F9_OA008_CRE_TMS BETWEEN ' || STRT_TMS || ' AND ' ||P_END_TMS;
        EXECUTE IMMEDIATE SQL_STMT INTO V_CNT;
        IF LENGTH(V_HP) >= 10 AND V_CNT=1 THEN
          ...
          IF p_crd_prd IN ('W','P') OR SUBSTR(p_crd_pgm,3,1) = 'S' THEN--NEU LA THE MC World, MC Debit Signature, VISA Platinum
            V_SMS_DETAIL := 'The SCB '||V_CRD_NAME||' x'||V_LAST4DIGIT||' GD khong thanh cong tai '||trim(p_merc_name)||' do the chua duoc kich hoat. Chi tiet LH '||v_hotline_vip||' de duoc ho tro';
          ELSE
            V_SMS_DETAIL := 'The SCB '||V_CRD_NAME||' x'||V_LAST4DIGIT||' GD khong thanh cong tai '||trim(p_merc_name)||' do the chua duoc kich hoat. Chi tiet LH '||v_hotline_std||' de duoc ho tro';
          END IF;

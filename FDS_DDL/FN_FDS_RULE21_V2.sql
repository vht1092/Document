--------------------------------------------------------
--  DDL for Function FN_FDS_RULE21_V2
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDS_RULE21_V2" 
/************************************************************************************
...
BEGIN
    ...
    IF (V_POS_MOD = '01' OR V_POS_MOD = '81' OR V_POS_MOD = '10') AND P_REF_CDE NOT LIKE '%10%' AND P_REF_CDE NOT LIKE '%35%' THEN
        ...
        sql_stmt := 'SELECT COUNT(1) FROM FDS_TXN_DETAIL where f9_oa008_amt_req >= 0 AND f9_oa008_cre_tms >= ' || strt_tms ||' AND f9_oa008_cre_tms <= ' || p_end_tms ||' AND substr(fx_oa008_pos_mode,1,2) in (''81'',''01'',''10'') AND fx_oa008_used_pan = ''' || p_cardno || '''';

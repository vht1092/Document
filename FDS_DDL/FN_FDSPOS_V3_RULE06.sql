--------------------------------------------------------
--  DDL for Function FN_FDSPOS_V3_RULE06
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDSPOS_V3_RULE06" (
/************************************************************************************************
...
AS
   ...
   v_cnt number := 0;
begin
   select count(1) into v_counter from fds_pos_txn_v3 where tid = p_tid AND ngay_gd = p_date AND LOAI_THE IN ('JCB','CLUB','AMEX','DCI','CUP');

--------------------------------------------------------
--  DDL for Function FN_FDS_RULE42_V2
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDS_RULE42_V2" 
/*********************************************************************************
...
BEGIN
  ...
  IF P_TXN_AMT > 0 THEN
    STRT_TMS := TMS_ADD_SUBTRACT(0.17,P_END_TMS);--trong 10 phut(1/6)
    ...
    THEN
      ...
      IF v_cnt >= 20 THEN--THOA �K HIT RULE
        ...
        --------------------------------------
        L_CHECK_POINT := 3;
        SQL_STMT2 :='INSERT INTO CCPS.FDS_CASE_HIT_RULE_DETAIL SELECT ''' || CRE_TMS || ''', ''' || CRE_TMS || ''', ''BACKEND_USR'', ''' || CASE_ID || ''', '''||V_RULENAME||''', T.F9_OA008_CRE_TMS, T.FX_OA008_USED_PAN, T.F9_OA008_MCC, T.P9_OA008_SEQ, T.F9_OA008_DT, T.F9_OA008_TM FROM FDS_TXN_DETAIL T WHERE T.F9_OA008_AMT_REQ > 0 AND ((T.FX_OA008_GIVEN_RESP_CDE = ''05'' and trim(T.FX_OA008_REF_CDE) is null and FX_OA008_OFC_CDE = ''SIP '') OR (T.FX_OA008_GIVEN_RESP_CDE = ''05'' and T.FX_OA008_REF_CDE like ''%26%'') OR (T.FX_OA008_GIVEN_RESP_CDE = ''96'' and (T.FX_OA008_REF_CDE like ''%23%'' OR T.FX_OA008_REF_CDE like ''%24%''))) AND T.F9_OA008_CRE_TMS BETWEEN '||STRT_TMS||' AND '||P_END_TMS||' ORDER BY T.F9_OA008_CRE_TMS DESC';

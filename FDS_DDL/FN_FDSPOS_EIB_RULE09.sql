--------------------------------------------------------
--  DDL for Function FN_FDSPOS_EIB_RULE09
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDSPOS_EIB_RULE09" (

/************************************************************************************************
* VER    ID        DATE         ENHANCEMENT                                                     *
* -----  -------   ---------    ----------------------------------------------------------------*
* 1.0    TANVH1    16JUL2020    MID phát sinh ≥ 3 giao dịch có số tiền/giao dịch là bội số của 1 triệu trong 1 ngày.                            *
*************************************************************************************************/
   p_mid in varchar2,
   p_date in number
) RETURN VARCHAR2
...
begin
   FOR RSROW IN (select * from fds_pos_txn_v2 where mid = p_mid AND ngay_gd = p_date and MOD(so_tien_gd_goc,1000000) = 0 order by NGAY_TAO_GD asc)
   LOOP
     select count(1) into v_count--dem so luong giao dich trong 1ngay
     from fds_pos_txn_v2
     where MID = p_mid

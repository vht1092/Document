--------------------------------------------------------
--  DDL for Function FN_FDS_RULE33_V2
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDS_RULE33_V2" 
/*********************************************************************************
...
BEGIN
    ...
    IF P_TXN_AMT > 0 AND P_MCC <> 6011 AND P_REF_CDE NOT LIKE '%10%' AND P_REF_CDE NOT LIKE '%35%' THEN
      ...
      SQL_STMT := 'SELECT COUNT(1) FROM FDS_TXN_DETAIL T WHERE T.F9_OA008_AMT_REQ > 0 AND T.F9_OA008_MCC <> 6011 AND T.FX_OA008_USED_PAN = '''||P_CARDNO||''' AND T.FX_OA008_TID = '''||P_TID||''' AND T.F9_OA008_CRE_TMS BETWEEN ' || STRT_TMS || ' AND ' ||P_END_TMS;
      ...
      IF V_CNT >= 3 THEN
          ...
          --------------------------------------
          L_CHECK_POINT := 3;
          SQL_STMT2 :='INSERT INTO CCPS.FDS_CASE_HIT_RULE_DETAIL SELECT '''||CRE_TMS||''', '''||CRE_TMS||''', ''BACKEND_USR'', ''' || CASE_ID || ''', '''||V_RULENAME||''', F9_OA008_CRE_TMS, FX_OA008_USED_PAN, F9_OA008_MCC, P9_OA008_SEQ, F9_OA008_DT, F9_OA008_TM FROM FDS_TXN_DETAIL WHERE F9_OA008_MCC <> 6011 AND FX_OA008_USED_PAN = '''|| P_CARDNO ||''' AND FX_OA008_TID = '''||P_TID||''' AND F9_OA008_CRE_TMS BETWEEN ' || STRT_TMS || ' AND ' ||P_END_TMS|| ' ORDER BY F9_OA008_CRE_TMS DESC';

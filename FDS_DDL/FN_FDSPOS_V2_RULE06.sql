--------------------------------------------------------
--  DDL for Function FN_FDSPOS_V2_RULE06
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDSPOS_V2_RULE06" (
/************************************************************************************************
* VER    ID        DATE         ENHANCEMENT                                                     *
* -----  -------   ---------    ----------------------------------------------------------------*
* 2.0    HUYENNT   12SEP2018    TID CO GD CUA CAC THE CLUBS, AMEX, DISCOVER, CUP, JCB           *
*************************************************************************************************/
   p_tid in varchar2,
   p_date in number
) RETURN VARCHAR2
AS
   ...
   v_cnt number := 0;
begin
   select count(1) into v_counter from fds_pos_txn_v2 where tid = p_tid AND ngay_gd = p_date AND LOAI_THE IN ('JCB','CLUB','AMEX','DCI','CUP');
   if v_counter >= v_so_lan then
     FOR RSROW IN (select MA_GD from fds_pos_txn_v2
       where tid = p_tid
           AND ngay_gd = p_date
           AND LOAI_THE IN ('JCB','CLUB','AMEX','DCI','CUP')
       order by
           NGAY_TAO_GD asc
     )
     LOOP

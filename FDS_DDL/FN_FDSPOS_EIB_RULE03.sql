--------------------------------------------------------
--  DDL for Function FN_FDSPOS_EIB_RULE03
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDSPOS_EIB_RULE03" (

/************************************************************************************************
* VER    ID        DATE         ENHANCEMENT                                                     *
* -----  -------   ---------    ----------------------------------------------------------------*
* 1.0    TANVH1    16JUL2020    MID phát sinh giao dịch có số tiền lớn ≥ 100 triệu/giao dịch HOẶC ngoại tệ  ≥ 4500 USD/giao dịch (áp dụng cho ĐVCNT được phép giao dịch bằng ngoại tệ) *
*************************************************************************************************/
   p_mid in varchar2,
   p_date in number
) RETURN VARCHAR2
...
begin
   ...
   where MID = p_mid     
     AND ngay_gd = p_date
     AND ((SO_TIEN_GD_GOC>=100000000 AND LOAI_TIEN='VND') OR (SO_TIEN_GD_GOC>=4500 AND LOAI_TIEN='USD'));

--------------------------------------------------------
--  DDL for Function FN_FDS_RULE36
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDS_RULE36" 
/**********************************************************************************
 * VER    ID        DATE         ENHANCEMENT                                      *
 * -----  -------   ---------    -------------------------------------------------*
 * 1.0    CHAUTK   17/12/2016    RULE 36: THE NHAP SAI PIN TU 2 LAN TRO LEN       *
 ...
 *                               SO 70/BC-GS&XLTST&NHDT(NGUNG CANH BAO)           *
 **********************************************************************************/
(
...
BEGIN
    ...
    COMMIT;
    -------------------------------------------------
    IF p_REF_CDE LIKE '%04%' THEN
        SQL_STMT := 'SELECT PIN_TRIES FROM (SELECT F9_OA051_PIN_TRIES PIN_TRIES FROM OA051@AM WHERE PX_OA051_PAN = '''||P_CARDNO||''' UNION SELECT F9_OA059_PIN_TRIES PIN_TRIES FROM OA059@AM WHERE PX_OA059_PAN = '''||P_CARDNO||''')';

--------------------------------------------------------
--  DDL for Function FN_FDSPOS_EIB_RULE04
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDSPOS_EIB_RULE04" (
/************************************************************************************************
* VER    ID        DATE         ENHANCEMENT                                                     *
* -----  -------   ---------    ----------------------------------------------------------------*
* 1.0    TANVH1    16JUL2020    MID phát sinh ≥ 2 giao dịch thẻ nội địa có số tiền ≥ 20 triệu/giao dịch trong 1 ngày                        *
*************************************************************************************************/
   p_mid in varchar2,
   p_date in number
) RETURN VARCHAR2
AS
   v_counter number := 0;
   v_so_lan number := 2;
   v_so_tien number := 20000000;
   v_rule_id varchar2(20):= 'RULE04';
   v_cre_tms number;
   v_cnt number := 0;
   V_CASE_NO varchar2(15);
begin
   select count(1) into v_counter from fds_pos_txn_v2 where mid = p_mid AND ngay_gd = p_date AND SO_THE LIKE '9%' AND so_tien_gd_goc >= v_so_tien;
   if v_counter >= v_so_lan then
     ...
     LOOP
       ...
       select
          RSROW.MA_GD,v_rule_id,V_CASE_NO, mid, ten_mid, tid, so_hd, dia_chi_gd,
          ngay_tao_gd, ngay_gio_gd, so_hoa_don, ma_chuan_chi,
          so_tien_gd_goc, loai_tien, so_tien_tip, so_bin,
          so_the, loai_the, ket_qua_gd, dao_huy, pos_mde_2digit,
          pos_mode, ma_loi, bao_co, ngay_gd, mcc

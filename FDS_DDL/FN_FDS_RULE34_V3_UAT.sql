--------------------------------------------------------
--  DDL for Function FN_FDS_RULE34_V3_UAT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDS_RULE34_V3_UAT" 
/**********************************************************************************
...
BEGIN
    ...
    IF p_ref_cde LIKE '%28%' AND p_ref_cde NOT LIKE '%10%' AND p_ref_cde NOT LIKE '%35%' AND (V_BIN = '510235' OR V_BIN = '545579' OR V_BIN = '489516' OR V_BIN = '489517') THEN
		...
		SQL_STMT := 'SELECT count(1) FROM FPT.FDS_TXN_DETAIL T WHERE T.fx_oa008_ref_cde LIKE ''%28%'' AND T.FX_OA008_USED_PAN = '''||P_CARDNO||''' AND T.F9_OA008_CRE_TMS BETWEEN ' || STRT_TMS || ' AND ' ||P_END_TMS;
		...
		IF V_CNT > 1 THEN--tu 2 giao dich tro len moi hit rule tao case
			...
			--------------------------------------
			L_CHECK_POINT := 3;
			SQL_STMT2 :='INSERT INTO FPT.FDS_CASE_HIT_RULE_DETAIL SELECT ''' || CRE_TMS || ''', ''' || CRE_TMS || ''', ''BACKEND_USR'', ''' || CASE_ID || ''', '''||V_RULENAME||''', F9_OA008_CRE_TMS, FX_OA008_USED_PAN, F9_OA008_MCC, P9_OA008_SEQ, F9_OA008_DT, F9_OA008_TM FROM FDS_TXN_DETAIL WHERE F9_OA008_CRE_TMS = '|| P_END_TMS ||' AND FX_OA008_USED_PAN = ''' || P_CARDNO || '''';
...
--              sms_scb.PROC_INS_MASTERCARD_SMS@eb_link(v_id_alert,'0963137131',V_SMS_DETAIL,V_ACT_TYP,V_SMS_TYP);
              INSERT INTO FPT.SMS_FDS_UAT(CRE_TMS,RULE_NAME,PHONE,SMS_DETAIL,PAN,MID,TXN_TMS) VALUES (to_char(SYSDATE, 'yyyyMMddHH24MISS'),'RULE34',V_HP,V_SMS_DETAIL,p_cardno,P_MID,p_end_tms);
			  -------------------------
			  COMMIT;
			  p_cnt:= 1;
			  return 'FALSE';

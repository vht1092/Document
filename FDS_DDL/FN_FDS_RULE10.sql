--------------------------------------------------------
--  DDL for Function FN_FDS_RULE10
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDS_RULE10" 
/*********************************************************************************
* VER    ID        DATE         ENHANCEMENT                                      *
* -----  -------   ---------    -------------------------------------------------*
* 1.0    HUYENNT   19DEC2016    THE PHAT SINH GIAO DICH TU 4 MCC KHAC NHAU TRONG *
*                               VONG 1H                                          *
...
(
 ...
 AS
    ...
    CASE_ID VARCHAR2(50);
    --BIEN DUNG GHI LOG--
    V_PROD VARCHAR(255):= 'SP_FDS_RULE10';
    V_RULENAME VARCHAR2(6):='RULE10';
...
BEGIN
    ...
    IF P_TXN_AMT > 0 THEN
      ...
      SQL_STMT := 'SELECT COUNT(DISTINCT T.F9_OA008_MCC) FROM FDS_TXN_DETAIL T WHERE T.f9_oa008_amt_req > 0 AND T.FX_OA008_USED_PAN = '''||P_CARDNO||''' AND T.F9_OA008_CRE_TMS BETWEEN '||STRT_TMS||' AND '||P_END_TMS;
      ...
      IF V_CNT >= 4 THEN
          ...
          --------------------------------------
          L_CHECK_POINT := 3;
          SQL_STMT2 :='INSERT INTO FDS_CASE_HIT_RULE_DETAIL SELECT ''' || CRE_TMS || ''', ''' || CRE_TMS || ''', ''BACKEND_USR'', ''' || CASE_ID || ''', '''||V_RULENAME||''', T.F9_OA008_CRE_TMS, T.FX_OA008_USED_PAN, T.F9_OA008_MCC, T.P9_OA008_SEQ, T.F9_OA008_DT, T.F9_OA008_TM FROM FDS_TXN_DETAIL T WHERE T.F9_OA008_AMT_REQ > 0 AND T.FX_OA008_USED_PAN = ''' || P_CARDNO || ''' AND T.F9_OA008_CRE_TMS BETWEEN '||STRT_TMS||' AND '||P_END_TMS||' ORDER BY T.F9_OA008_CRE_TMS DESC';

--------------------------------------------------------
--  DDL for Function FN_FDS_RULE32
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDS_RULE32" 
/*********************************************************************************
* VER    ID        DATE         ENHANCEMENT                                      *
* -----  -------   ---------    -------------------------------------------------*
* 1.0    CHAUTK   19DEC2016     TRONG VONG 12H, THE CO TU 3 GIAO DICH TRO LEN TAI*
*                               CUNG 1 TERMINAL                                  *
...
(
    ...
    P_CRD_PGM IN CHAR,--ADD ON 27122017
    P_END_TMS IN VARCHAR2,
    P_CHECK IN VARCHAR2,
    P_TID IN VARCHAR2,--TERMINAL ID
    P_TIME_PERIOD IN NUMBER,-- TRUYEN VAO 12
    P_CNT OUT NUMBER
 ) RETURN VARCHAR2
 AS
    ...
    CASE_ID VARCHAR2(50);
    --BIEN DUNG GHI LOG--
    V_PROD VARCHAR(255):= 'FN_FDS_RULE32';
    V_RULENAME VARCHAR2(6):='RULE32';
...
BEGIN
    ...
    IF P_TXN_AMT > 0 THEN
      ...
      SQL_STMT := 'SELECT COUNT(1) FROM FDS_TXN_DETAIL T WHERE T.F9_OA008_AMT_REQ > 0 AND T.FX_OA008_USED_PAN = '''||P_CARDNO||''' AND T.FX_OA008_TID = '''||P_TID||''' AND T.F9_OA008_CRE_TMS BETWEEN ' || STRT_TMS || ' AND ' ||P_END_TMS;
      EXECUTE IMMEDIATE SQL_STMT INTO V_CNT;
      IF (V_CNT >= 7 AND P_CRD_PGM LIKE 'MD%') OR (V_CNT >= 5 AND P_CRD_PGM NOT LIKE 'MD%') THEN
          L_CHECK_POINT := 2;
          ...
          --------------------------------------
          L_CHECK_POINT := 3;
          SQL_STMT2 :='INSERT INTO FDS_CASE_HIT_RULE_DETAIL SELECT '''||CRE_TMS||''', '''||CRE_TMS||''', ''BACKEND_USR'', ''' || CASE_ID || ''', '''||V_RULENAME||''', T.F9_OA008_CRE_TMS, T.FX_OA008_USED_PAN, T.F9_OA008_MCC, T.P9_OA008_SEQ, T.F9_OA008_DT, T.F9_OA008_TM FROM FDS_TXN_DETAIL T WHERE T.FX_OA008_USED_PAN = '''|| P_CARDNO ||''' AND T.FX_OA008_TID = '''||P_TID||''' AND T.F9_OA008_CRE_TMS BETWEEN ' || STRT_TMS || ' AND ' ||P_END_TMS|| ' ORDER BY T.F9_OA008_CRE_TMS DESC';

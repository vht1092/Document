--------------------------------------------------------
--  DDL for Function FN_FDS_RULE39
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDS_RULE39" 
/*********************************************************************************
...
(
 ...
 AS
    ...
    CASE_ID VARCHAR2(50);
    V_AMOUNT_CR NUMBER := 5000000; -- SO TIEN GIAO DICH THE CREDIT
    V_AMOUNT_DB NUMBER := 2000000; -- SO TIEN GIAO DICH THE DEBIT    
    --BIEN DUNG GHI LOG--
...
BEGIN
    ...
    -------------------------------------------------
    IF ((TXN_TMS >= 2330 and TXN_TMS <= 2359) OR (TXN_TMS >= 0 and TXN_TMS <= 400)) AND TRIM(P_CRCY_CDE) = '704' AND ((P_TXN_AMT >= V_AMOUNT_CR AND P_CRD_PGM NOT LIKE 'MD%') OR (P_TXN_AMT >= V_AMOUNT_DB AND P_CRD_PGM LIKE 'MD%')) AND P_MCC NOT IN (/*'5812', */'5813',/* '5715',*/ '5921') THEN
        L_CHECK_POINT := 2;

--------------------------------------------------------
--  DDL for Function FN_FDS_RULE22
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDS_RULE22" 
/*********************************************************************************
* VER    ID        DATE         ENHANCEMENT                                      *
* -----  -------   ---------    -------------------------------------------------*
* 1.0    HUYENNT   19DEC2016    The gd tai cac tiem vang va co so tien gd tu 20tr*
*                               VND hoac ngoai te tuong duong THE CHUAN, 40TR VND*
*                               HOAC NGOAI TE TUONG DUONG VOI THE VANG PLATINUM  *
*                               TAI CAC MCC 5094, 5944, 5950, 7631               *
...
(
    ...
    P_MCC IN NUMBER,--ADD ON 21022017
 ...
 AS
    ...
    cre_tms varchar(17);
    case_id VARCHAR2(50);
    V_AMT_R number := 20000000;
    V_AMT_G number := 40000000;
    --BIEN DUNG GHI LOG--
    v_PROD varchar(255);
...
BEGIN
    ...
    COMMIT;
    -------------------------------------------------
    IF P_TXN_AMT >= V_AMT_R AND P_CRD_PRD = 'R' AND P_MCC in (5094, 5944, 5950, 7631) THEN
        l_check_point := 2;
        ...
        ------------------------------------
        l_check_point := 3;
        sql_stmt2 := 'INSERT INTO FDS_CASE_HIT_RULE_DETAIL SELECT ''' || cre_tms || ''',''' || cre_tms || ''',''BACKEND_USR'',''' || case_id || ''',''RULE22'',f9_oa008_cre_tms,fx_oa008_used_pan,f9_oa008_mcc,p9_oa008_seq,f9_oa008_dt,f9_oa008_tm FROM FDS_TXN_DETAIL where f9_oa008_cre_tms = ' || p_end_tms ||' AND fx_oa008_used_pan = ''' || p_cardno || '''';
        execute immediate sql_stmt2;
        l_check_point := 4;
        sql_stmt2 := 'INSERT INTO FDS_CASE_HIT_RULES VALUES(' || cre_tms || ',' || cre_tms || ',''BACKEND_USR'',''' || case_id || ''',''RULE22'')';
        execute immediate sql_stmt2;
        commit;
        p_cnt := 1;
        RETURN 'DONE';
    ELSIF P_TXN_AMT >= V_AMT_G AND P_CRD_PRD <> 'R' AND P_MCC in (5094, 5944, 5950, 7631) THEN
        l_check_point := 2;
        SELECT TO_CHAR(SYSDATE,'yyyymmddhh24misssss') INTO CRE_TMS FROM dual;
        ----lay dung cif theo the chinh hoac phu
        select count(1) INTO V_CRN from oa059@am where PX_OA059_PAN = P_CARDNO;

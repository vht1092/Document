--------------------------------------------------------
--  DDL for Function FN_FDS_RULE23_V2_TEST
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDS_RULE23_V2_TEST" 
/*********************************************************************************
...
* 3.0    HUYENNT   08AUG2019    CHECK THEO MERC_ST_CNTRY <> ' ' VA TONG SO TIEN  *
*                               GIAO DICH >= 2TR                                 *
**********************************************************************************/
(
 ...
 AS
    ...
    ---------------------
    v_total_txn_amt number;--ADD ON 08AUG2019
    v_cnt1 number;
    v_total_txn_amt1 number;
BEGIN
    ...
    IF (V_POS_MODE = '05' OR V_POS_MODE = '90' OR V_POS_MODE = '91') AND P_REF_CDE NOT LIKE '%10%' AND P_REF_CDE NOT LIKE '%35%' THEN
        ...
        l_check_point := 2;
        SELECT
          COUNT(DISTINCT CNTR_CDE), sum(so_tien_gd) INTO v_cnt, v_total_txn_amt
        FROM
          (SELECT
              nvl(nvl((case when trim(fx_oa008_merc_st_cntry) = '04' then 'VNM' when trim(fx_oa008_merc_st_cntry) = '70' then 'VNM' when fx_oa008_merc_st_cntry = '704' then 'VNM' when upper(trim(fx_oa008_merc_st_cntry)) = 'VN' then 'VNM' else null end),(select ISO_COUNTRY_CODE from iso_uscan_province_code where ISO_PROVINCE_CODE = upper(trim(fx_oa008_merc_st_cntry)))),upper(fx_oa008_merc_st_cntry)) AS CNTR_CDE,
              f9_oa008_amt_req as so_tien_gd
          FROM
              fds_txn_detail
          where
              substr(fx_oa008_pos_mode,1,2) in ('05','90','91')
              AND FX_OA008_MERC_ST_CNTRY <> '   '
        ...
        if v_cnt1 > 0 then
          SELECT
              count(distinct f9_oa008_crncy_cde),sum(f9_oa008_amt_req) into v_cnt1,v_total_txn_amt1
          FROM
              fds_txn_detail
          where
              substr(fx_oa008_pos_mode,1,2) in ('05','90','91')
        ...
        ------------------------
        IF (v_cnt >= 2 and v_total_txn_amt >= 2000000) OR (v_cnt1 >= 2 and v_total_txn_amt1 >= 2000000) THEN
            l_check_point := 3;
            ...
            ------------------------------------
            l_check_point := 4;
            sql_stmt2 := 'INSERT INTO FPT.FDS_CASE_HIT_RULE_DETAIL SELECT ''' || cre_tms || ''',''' || cre_tms || ''',''BACKEND_USR'',''' || case_id || ''',''RULE23'',f9_oa008_cre_tms,fx_oa008_used_pan,f9_oa008_mcc,p9_oa008_seq,f9_oa008_dt,f9_oa008_tm FROM FDS_TXN_DETAIL where substr(fx_oa008_pos_mode,1,2) in (''05'',''90'',''91'') AND f9_oa008_cre_tms >= ' || strt_tms ||' AND f9_oa008_cre_tms <= ' || p_end_tms || ' AND fx_oa008_used_pan = ''' || p_cardno || ''' ORDER BY f9_oa008_cre_tms desc';
            execute immediate sql_stmt2;
            l_check_point := 5;
            sql_stmt2 := 'INSERT INTO FPT.FDS_CASE_HIT_RULES VALUES(' || cre_tms || ',' || cre_tms || ',''BACKEND_USR'',''' || case_id || ''',''RULE23'')';
            execute immediate sql_stmt2;
            commit;
            p_cnt := 1;
            RETURN 'DONE';

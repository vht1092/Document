--------------------------------------------------------
--  DDL for Function FN_FDSPOS_V2_RULE10
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDSPOS_V2_RULE10" (

/************************************************************************************************
* VER    ID        DATE         ENHANCEMENT                                                     *
* -----  -------   ---------    ----------------------------------------------------------------*
* 2.0    HUYENNT   12SEP2018    TID CO NHIEU GIAO VOI SO TIEN GIONG NHAU                        *
*************************************************************************************************/
   p_tid in varchar2,
   p_date in number
) RETURN VARCHAR2
AS
   ...
   v_end_tms number;
   v_so_lan number := 3;
   v_rule_id varchar2(20):= 'RULE10';
   v_cre_tms number;
   v_cnt number := 0;
begin
   FOR RSROW IN (select * from fds_pos_txn_v2 where tid = p_tid AND ngay_gd = p_date order by NGAY_TAO_GD asc)
   LOOP
     ...
     from fds_pos_txn_v2
     where TID = p_tid
           and so_tien_gd_goc = RSROW.SO_TIEN_GD_GOC

--------------------------------------------------------
--  DDL for Function FN_FDS_RULE03_V2
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDS_RULE03_V2" 
/*********************************************************************************
...
(
 ...
 AS
    ...
    V_AMOUNT90_R NUMBER:=20000000; -- SO TIEN GIAO DICH TU THE CHUAN
    V_AMOUNT90_G NUMBER:=30000000; -- SO TIEN GIAO DICH TU THE VANG
    V_AMOUNT90_PW NUMBER:=50000000; -- SO TIEN GIAO DICH TU THE P,W
    --BIEN DUNG GHI LOG--
...
BEGIN
    ...
    IF P_POS_MODE LIKE '05%' AND P_REF_CDE NOT LIKE '%10%' AND P_REF_CDE NOT LIKE '%35%' THEN--GD CHIP
      ...
      ELSE
          ...
          RETURN 'FALSE';
      END IF;
    ELSIF P_POS_MODE LIKE '90%' AND P_REF_CDE NOT LIKE '%10%' AND P_REF_CDE NOT LIKE '%35%' THEN--GD TU
      IF P_TXN_AMT > 0 THEN
        STRT_TMS := TMS_ADD_SUBTRACT(P_TIME_PERIOD,P_END_TMS);
        SQL_STMT := 'SELECT SUM(F9_OA008_AMT_REQ) FROM FDS_TXN_DETAIL T WHERE T.F9_OA008_AMT_REQ > 0 AND T.FX_OA008_POS_MODE LIKE ''90%'' AND T.FX_OA008_USED_PAN = '''||P_CARDNO||''' AND T.F9_OA008_CRE_TMS BETWEEN ' || STRT_TMS || ' AND ' ||P_END_TMS;
        EXECUTE IMMEDIATE SQL_STMT INTO V_CNT;
        IF (P_CRD_PRD = 'R' AND V_CNT >= V_AMOUNT90_R) OR (P_CRD_PRD = 'G' AND V_CNT >= V_AMOUNT90_G) OR (P_CRD_PRD IN ('P','W') AND V_CNT >= V_AMOUNT90_PW) THEN
            ...
            --------------------------------------
            L_CHECK_POINT := 3;
            SQL_STMT2 :='INSERT INTO CCPS.FDS_CASE_HIT_RULE_DETAIL SELECT ''' || CRE_TMS || ''', ''' || CRE_TMS || ''', ''BACKEND_USR'', ''' || CASE_ID || ''', '''||V_RULENAME||''', T.F9_OA008_CRE_TMS, T.FX_OA008_USED_PAN, T.F9_OA008_MCC, T.P9_OA008_SEQ, T.F9_OA008_DT, T.F9_OA008_TM FROM FDS_TXN_DETAIL T WHERE T.F9_OA008_AMT_REQ > 0 AND T.FX_OA008_POS_MODE LIKE ''90%'' AND T.FX_OA008_USED_PAN = ''' || P_CARDNO || ''' AND T.F9_OA008_CRE_TMS BETWEEN '||STRT_TMS||' AND '||P_END_TMS||' ORDER BY T.F9_OA008_CRE_TMS DESC';
            EXECUTE IMMEDIATE SQL_STMT2;
            L_CHECK_POINT := 4;
            SQL_STMT2 := 'INSERT INTO CCPS.FDS_CASE_HIT_RULES VALUES(' || CRE_TMS || ',' || CRE_TMS || ',''BACKEND_USR'',''' || CASE_ID || ''','''||V_RULENAME||''')';
            EXECUTE IMMEDIATE SQL_STMT2;

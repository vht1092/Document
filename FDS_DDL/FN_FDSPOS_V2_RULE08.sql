--------------------------------------------------------
--  DDL for Function FN_FDSPOS_V2_RULE08
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDSPOS_V2_RULE08" (

/************************************************************************************************
* VER    ID        DATE         ENHANCEMENT                                                     *
* -----  -------   ---------    ----------------------------------------------------------------*
* 2.0    HUYENNT   12SEP2018    TID CO NHIEU GIAO DICH BI TU CHOI                               *
*************************************************************************************************/
   p_tid in varchar2,
   p_date in number
) RETURN VARCHAR2
AS
   ...
   v_end_tms number;
   v_so_lan number := 5;
   v_rule_id varchar2(20):= 'RULE08';
   v_cre_tms number;
   v_cnt number := 0;
begin
   FOR RSROW IN (select * from fds_pos_txn_v2 where tid = p_tid AND ngay_gd = p_date and substr(KET_QUA_GD,1,1) = 'L' order by NGAY_TAO_GD asc)
   LOOP

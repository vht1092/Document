--------------------------------------------------------
--  DDL for Function FN_FDSPOS_V2_RULE18
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDSPOS_V2_RULE18" (
/************************************************************************************************
* VER    ID        DATE         ENHANCEMENT                                                     *
* -----  -------   ---------    ----------------------------------------------------------------*
* 2.0    HUYENNT   19SEP2018    TID PHAT SINH GD SAU NHIEU THANG KHONG HOAT DONG                *
*************************************************************************************************/
   p_tid in varchar2,
   p_date in number
) RETURN VARCHAR2
AS
   ...
   v_rule_id varchar2(20):= 'RULE18';
   v_cre_tms number;
   v_cnt number := 0;
   V_MA_GD NUMBER;
begin
   select MIN(MA_GD) into V_MA_GD--LAY MA GD TU TID
   from fds_pos_txn_v2
   where TID = p_tid
         and ngay_gd = p_date;
   select COUNT(1) into v_total_amtof7day--SO LUONG GIAO DICH PHAT SINH TRONG 2 THANG TRUOC CUA TID
   from fds_pos_txn_v2
   where TID = p_tid
         and ngay_gd BETWEEN to_char(add_months(to_date(p_date, 'yyyymmdd') - 1,-2),'yyyymmdd') AND TO_CHAR(TO_DATE(p_date,'YYYYMMDD')-1,'YYYYMMDD');
   IF v_total_amtof7day <= 5 THEN

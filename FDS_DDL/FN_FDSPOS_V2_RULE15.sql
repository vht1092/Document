--------------------------------------------------------
--  DDL for Function FN_FDSPOS_V2_RULE15
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDSPOS_V2_RULE15" (
/************************************************************************************************
* VER    ID        DATE         ENHANCEMENT                                                     *
* -----  -------   ---------    ----------------------------------------------------------------*
* 2.0    HUYENNT   19SEP2018    TID CO DOANH SO/NGAY GIAM BAT THUONG                            *
*************************************************************************************************/
   p_tid in varchar2,
   p_date in number
) RETURN VARCHAR2
AS
   ...
   v_rule_id varchar2(20):= 'RULE15';
   v_cre_tms number;
   v_cnt number := 0;
   V_MA_GD NUMBER;
begin
   select sum(so_tien_gd_goc), MIN(MA_GD) into v_total_amt, V_MA_GD--DOANH SO NGAY CUA TID
   from fds_pos_txn_v2
   where TID = p_tid
         and ngay_gd = p_date;
   select ROUND(sum(so_tien_gd_goc)/7) into v_total_amtof7day--DOANH SO TRUNG BINH 7 NGAY LIEN KE CUA TID
   from fds_pos_txn_v2
   where TID = p_tid
         and ngay_gd BETWEEN TO_CHAR(TO_DATE(p_date,'YYYYMMDD')-8,'YYYYMMDD') AND TO_CHAR(TO_DATE(p_date,'YYYYMMDD')-1,'YYYYMMDD');
   IF v_total_amt/v_total_amtof7day <= 3/2 THEN--DOANH SO NGAY GIAM <= 150% DOANH SO TB CUA 7 NGAY LIEN KE

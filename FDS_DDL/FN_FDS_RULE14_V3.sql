--------------------------------------------------------
--  DDL for Function FN_FDS_RULE14_V3
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDS_RULE14_V3" 
/***********************************************************************************
...
BEGIN
    ...
    FROM
        oa063@IM
    WHERE
       px_oa063_pan IN (P_CARDNO,V_PAN);
    --------------------------------------------------*/
    ...
    IF p_ref_cde LIKE '%20%' AND p_ref_cde NOT LIKE '%10%' AND p_ref_cde NOT LIKE '%35%'/* OR v_block_ind > 0*/ THEN
		...
		IF V_CNT > 0 THEN
			...
			SELECT trim(p9_oa008_seq)||'-'||trim(fx_oa008_given_apv_cde)||'-'||trim(f9_oa008_stan)||'-'||trim(f9_oa008_mcc) INTO case_id FROM fds_txn_detail where f9_oa008_amt_req > 0 AND fx_oa008_used_pan IN (P_CARDNO,V_PAN) AND f9_oa008_cre_tms = p_end_tms AND P9_OA008_SEQ=P_SEQ;
			...
			--------------------------------------
			l_check_point := 3;
			sql_stmt2 := 'INSERT INTO CCPS.FDS_CASE_HIT_RULE_DETAIL SELECT ''' || cre_tms || ''',''' || cre_tms || ''',''BACKEND_USR'',''' || case_id || ''',''RULE14'',f9_oa008_cre_tms,fx_oa008_used_pan,f9_oa008_mcc,p9_oa008_seq,f9_oa008_dt,f9_oa008_tm FROM FDS_TXN_DETAIL where f9_oa008_cre_tms = ' || p_end_tms ||' AND fx_oa008_used_pan = ''' || p_cardno || '''';
			execute immediate sql_stmt2;
			l_check_point := 4;
			sql_stmt2 := 'INSERT INTO CCPS.FDS_CASE_HIT_RULES VALUES(' || cre_tms || ',' || cre_tms || ',''BACKEND_USR'',''' || case_id || ''',''RULE14'')';
			execute immediate sql_stmt2;
			commit;
		...
		ELSE
            ...
            SQL_STMT := 'SELECT count(1) FROM CCPS.FDS_TXN_DETAIL T WHERE T.FX_OA008_USED_PAN = '''||P_CARDNO||''' AND T.FX_OA008_MID = '''||P_MID||''' AND T.F9_OA008_CRE_TMS BETWEEN ' || STRT_TMS || ' AND ' ||P_END_TMS;
            EXECUTE IMMEDIATE SQL_STMT INTO V_CNT;
...
--              INSERT INTO CCPS.SMS_FDS(CRE_TMS,RULE_NAME,PHONE,SMS_DETAIL,PAN,MID,TXN_TMS) VALUES (to_char(SYSDATE, 'yyyyMMddHH24MISS'),'RULE16',V_HP,V_SMS_DETAIL,p_cardno,P_MID,p_end_tms);
			  -------------------------
			  COMMIT;
			  p_cnt:= 1;
			  return 'FALSE';

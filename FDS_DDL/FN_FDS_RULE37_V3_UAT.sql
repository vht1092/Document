--------------------------------------------------------
--  DDL for Function FN_FDS_RULE37_V3_UAT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDS_RULE37_V3_UAT" 
/**********************************************************************************
...
(
    ...
    P_MCC IN NUMBER,--ADD ON 20200116
...
BEGIN
    ...
    COMMIT;
    SQL_STMT := 'SELECT TRIM(P9_OA008_SEQ)||''-''||TRIM(FX_OA008_GIVEN_APV_CDE)||''-''||TRIM(F9_OA008_STAN)||''-''||TRIM(F9_OA008_MCC) FROM FPT.FDS_TXN_DETAIL WHERE FX_OA008_USED_PAN = '''||P_CARDNO||''' AND F9_OA008_CRE_TMS ='|| P_END_TMS || ' AND P9_OA008_SEQ=' || P_SEQ;
    EXECUTE IMMEDIATE SQL_STMT INTO CASE_ID;
    SQL_STMT := 'SELECT COUNT(1) FROM FPT.FDS_CASE_HIT_RULES WHERE rule_id IN (''RULE13'',''RULE14'',''RULE15'',''RULE16'',''RULE20'',''RULE34'',''RULE35'',''RULE43'') AND CASE_NO = '''||CASE_ID||'''';
    EXECUTE IMMEDIATE SQL_STMT INTO V_CNT;
	SELECT PX_OA008_PAN INTO V_PAN FROM FPT.FDS_TXN_DETAIL WHERE FX_OA008_USED_PAN=P_CARDNO AND P9_OA008_SEQ=P_SEQ AND F9_OA008_CRE_TMS=P_END_TMS;
    IF V_CNT = 0--khong co trong cac rule da hit theo danh sach loai tru o tren
       AND (P_REF_CDE LIKE '%01%'
            ...
            OR (P_REF_CDE LIKE '%34%' AND P_MCC=6011)
...
--            OR P_REF_CDE LIKE '%39%'
            ...
            AND P_REF_CDE NOT LIKE '%28%' --AND (P_REF_CDE LIKE '%34%' AND P_REF_CDE<>6011)
    ...
    THEN
        ...
        END IF;
        ----------------------------------------
        SELECT trim(p9_oa008_seq)||'-'||trim(fx_oa008_given_apv_cde)||'-'||trim(f9_oa008_stan)||'-'||trim(f9_oa008_mcc) INTO case_id FROM FPT.fds_txn_detail where f9_oa008_amt_req >= 0 AND fx_oa008_used_pan = p_cardno AND f9_oa008_cre_tms = p_end_tms;
        ...
        IF p_check = 'FALSE' THEN
            SQL_STMT := 'INSERT INTO FPT.FDS_CASE_DETAIL (CRE_TMS,UPD_TMS,
                                 ...
                                 CRN_LMT_AVL,
                                 CRN)
             SELECT ''' || CRE_TMS || ''', ''' || CRE_TMS || ''', ''BACKEND_USR'', 0, ''BACKEND_USR'', '' '', '''|| V_CIF ||''' , '''|| CASE_ID ||''', F9_OA008_ORI_AMT, 0, 0, ''NEW'', ''Y'', ''N'', FX_OA008_USED_PAN, F9_OA008_MCC, 0, F9_OA008_CRE_TMS, FX_OA008_CRD_BRN, FX_OA008_MERC_NAME, F9_OA008_CRNCY_CDE, FX_OA008_POS_MODE, DECODE(FX_OA008_REF_CDE, '' '', ''00'', DECODE(FX_OA008_CRD_BRN, ''MC'', (SELECT FX_OA274_MC_RESP_CDE FROM OA274@IM WHERE TRIM(PX_OA274_REF_CDE) = FX_OA008_REF_CDE), (SELECT FX_OA274_VS_RESP_CDE FROM OA274@IM WHERE TRIM(PX_OA274_REF_CDE) = FX_OA008_REF_CDE))) AS RESP_CDE, FX_OA008_STAT,  '''|| V_3D_IND ||''' , '''|| V_3D_ECI ||''', F9_OA008_ACCT_NUM,0,0,0,'||P_CRN||' FROM (SELECT T.F9_OA008_CRE_TMS, T.F9_OA008_PRIN_CRN, T.PX_OA008_PAN, T.F9_OA008_ACCT_NUM, T.FX_OA008_USED_PAN, T.P9_OA008_SEQ, T.FX_OA008_GIVEN_APV_CDE, T.F9_OA008_AMT_REQ, T.F9_OA008_ORI_AMT, T.F9_OA008_MCC, T.FX_OA008_CRD_BRN, T.FX_OA008_MERC_NAME, T.F9_OA008_CRNCY_CDE, T.FX_OA008_POS_MODE, T.FX_OA008_STAT, NVL(SUBSTR(TRIM(T.FX_OA008_REF_CDE), 1, 2), '' '') AS FX_OA008_REF_CDE FROM FPT.FDS_TXN_DETAIL T WHERE T.F9_OA008_CRE_TMS = ' || P_END_TMS ||' AND T.FX_OA008_USED_PAN = ''' || P_CARDNO ||''' AND T.P9_OA008_SEQ='||P_SEQ||')';
            execute immediate sql_stmt;
        END IF;
        --------------------------------------
        L_CHECK_POINT := 3;
        SQL_STMT2 :='INSERT INTO FPT.FDS_CASE_HIT_RULE_DETAIL SELECT ''' || CRE_TMS || ''', '''|| CRE_TMS ||''', ''BACKEND_USR'', '''|| CASE_ID ||''', '''||V_RULENAME||''', F9_OA008_CRE_TMS, FX_OA008_USED_PAN, F9_OA008_MCC, P9_OA008_SEQ, F9_OA008_DT, F9_OA008_TM FROM FPT.FDS_TXN_DETAIL WHERE F9_OA008_CRE_TMS = '|| P_END_TMS ||' AND FX_OA008_USED_PAN = '''|| P_CARDNO||'''';
    ...
    ELSE
		...
		SQL_STMT := 'SELECT count(1) FROM FPT.FDS_TXN_DETAIL T WHERE T.fx_oa008_ref_cde LIKE ''%28%'' AND T.FX_OA008_USED_PAN = '''||P_CARDNO||''' AND T.FX_OA008_MID = '''||P_MID||''' AND T.F9_OA008_CRE_TMS BETWEEN ' || STRT_TMS || ' AND ' ||P_END_TMS;
		EXECUTE IMMEDIATE SQL_STMT INTO V_CNT_GD_LON;

		SQL_STMT := 'SELECT count(1) FROM FPT.FDS_TXN_DETAIL T WHERE (T.fx_oa008_ref_cde LIKE ''%04%'' OR T.fx_oa008_ref_cde LIKE ''%33%'' ) AND T.FX_OA008_USED_PAN = '''||P_CARDNO||''' AND T.FX_OA008_MID = '''||P_MID||''' AND T.F9_OA008_CRE_TMS BETWEEN ' || STRT_TMS || ' AND ' ||P_END_TMS;
...
--			  sms_scb.PROC_INS_MASTERCARD_SMS@eb_link(v_id_alert,'0963137131',V_SMS_DETAIL,V_ACT_TYP,V_SMS_TYP);
            INSERT INTO FPT.SMS_FDS_UAT(CRE_TMS,RULE_NAME,PHONE,SMS_DETAIL,PAN,MID,TXN_TMS) VALUES (to_char(SYSDATE, 'yyyyMMddHH24MISS'),'RULE37',V_HP,V_SMS_DETAIL,p_cardno,P_MID,p_end_tms);
			  -------------------------
			  COMMIT;
			  p_cnt:= 1;
			  return 'FALSE';

--------------------------------------------------------
--  DDL for Function FN_FDS_RULE38
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDS_RULE38" 
/*********************************************************************************
* VER    ID        DATE         ENHANCEMENT                                      *
* -----  -------   ---------    -------------------------------------------------*
* 1.0    HUYENNT   04JAN2017    Giao dich the CREDIT MASTERCARD, VISA tai cac ATM*
*                               voi so tien tuong duong > 5 trieu VND/ giao dich *
...
BEGIN
    ...
    COMMIT;
    -------------------------------------------------
    IF p_mcc = 6011 AND substr(p_crd_pgm,1,2) <> 'MD' AND P_txn_amt > 5000000 THEN
        l_check_point := 2;
        ...
        END IF;
        ----CREATE A NEW CASE HIT RULE DETAIL-----------------------------
        l_check_point := 3;
        sql_stmt := 'INSERT INTO FDS_CASE_HIT_RULE_DETAIL SELECT ''' || cre_tms || ''',''' || cre_tms || ''',''BACKEND_USR'',''' || case_id || ''',''RULE38'',f9_oa008_cre_tms,fx_oa008_used_pan,f9_oa008_mcc,p9_oa008_seq,f9_oa008_dt,f9_oa008_tm FROM FDS_TXN_DETAIL where f9_oa008_cre_tms = ' || p_end_tms ||' AND fx_oa008_used_pan = ''' || p_cardno || '''';
        execute immediate sql_stmt;
        ----CREATE A NEW CASE HIT RULE-------------------------------
        l_check_point := 4;
        sql_stmt := 'INSERT INTO FDS_CASE_HIT_RULES VALUES(' || cre_tms || ',' || cre_tms || ',''BACKEND_USR'',''' || case_id || ''',''RULE38'')';
        execute immediate sql_stmt;
        commit;
        -----------------------------------
        p_cnt := 1;

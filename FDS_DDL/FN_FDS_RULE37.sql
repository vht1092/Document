--------------------------------------------------------
--  DDL for Function FN_FDS_RULE37
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDS_RULE37" 
/**********************************************************************************
...
BEGIN
    ...
    COMMIT;
    SQL_STMT := 'SELECT TRIM(P9_OA008_SEQ)||''-''||TRIM(FX_OA008_GIVEN_APV_CDE)||''-''||TRIM(F9_OA008_STAN)||''-''||TRIM(F9_OA008_MCC) FROM FDS_TXN_DETAIL WHERE F9_OA008_AMT_REQ >= 0 AND FX_OA008_USED_PAN = '''||P_CARDNO||''' AND F9_OA008_CRE_TMS ='|| P_END_TMS || ' AND P9_OA008_SEQ=' || P_SEQ;
    EXECUTE IMMEDIATE SQL_STMT INTO CASE_ID;
    SQL_STMT := 'SELECT COUNT(1) FROM FDS_CASE_HIT_RULES WHERE rule_id IN (''RULE13'',''RULE14'',''RULE15'',''RULE16'',''RULE34'',''RULE35'',''RULE19'',''RULE20'') AND CASE_NO = '''||CASE_ID||'''';

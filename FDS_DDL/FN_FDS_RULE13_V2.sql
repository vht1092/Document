--------------------------------------------------------
--  DDL for Function FN_FDS_RULE13_V2
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "CCPS"."FN_FDS_RULE13_V2" 
/***********************************************************************************
...
--	SELECT PX_OA008_PAN INTO V_PAN FROM FDS_TXN_DETAIL WHERE FX_OA008_USED_PAN=P_CARDNO AND P9_OA008_SEQ=P_SEQ AND F9_OA008_CRE_TMS=P_END_TMS;
    IF P_TXN_AMT > 0 AND p_ref_cde LIKE '%13%' THEN
        ...
        SQL_STMT := 'SELECT count(1) FROM FDS_TXN_DETAIL T WHERE T.F9_OA008_AMT_REQ > 0 AND T.fx_oa008_ref_cde LIKE ''%13%'' AND T.FX_OA008_USED_PAN = '''||P_CARDNO||''' AND T.F9_OA008_CRE_TMS BETWEEN ' || STRT_TMS || ' AND ' ||P_END_TMS;
        ...
        IF V_CNT > 1 THEN--tu 2 giao dich tro len moi hit rule tao case
          ...
          --------------------------------------
          l_check_point := 3;
          sql_stmt2 := 'INSERT INTO CCPS.FDS_CASE_HIT_RULE_DETAIL SELECT ''' || cre_tms || ''',''' || cre_tms || ''',''BACKEND_USR'',''' || case_id || ''',''RULE13'',f9_oa008_cre_tms,fx_oa008_used_pan,f9_oa008_mcc,p9_oa008_seq,f9_oa008_dt,f9_oa008_tm FROM FDS_TXN_DETAIL where f9_oa008_amt_req >= 0 and f9_oa008_cre_tms = ' || p_end_tms ||' AND fx_oa008_used_pan = ''' || p_cardno || '''';
        ...
        ELSE
          IF LENGTH(V_HP) >= 10 THEN
            ...
            V_SMS_DETAIL := 'The SCB '||V_CRD_NAME||' x'||V_LAST4DIGIT||' giao dich khong thanh cong tai '||trim(p_merc_name)||' do the khoa chuc nang chi tieu qua mang. Chi tiet LH (028)73025999 de duoc ho tro';
